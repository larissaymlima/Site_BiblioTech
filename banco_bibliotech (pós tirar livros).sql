...
CREATE TABLE leitores (
    ...
    telefone VARCHAR(15) NOT NULL,
    senha VARCHAR(255) NOT NULL,
    consentimento_lgpd BOOLEAN DEFAULT 0, -- Ajustado para BOOLEAN (sofrerá ajustes em próximas atualizações)
    foto_perfil VARCHAR(255) DEFAULT 'default_profile.png',
...
CREATE TABLE notificacoes_interesse (
    ...
    id_leitor INT NOT NULL,
    id_livro INT NOT NULL,
    consentimento_lgpd BOOLEAN DEFAULT 0, -- Ajustado para BOOLEAN (poderá sofrer alterações em outras atualizações)
...
AFTER INSERT ON emprestimos -- Executado LOGO APÓS inserir um novo registro em 'emprestimos'
FOR EACH ROW
BEGIN
    UPDATE livro  -- Atualiza a tabela 'livro' alterando o status do livro correspondente
    SET status_exemplar = 'Emprestado' 
    WHERE id_livro = NEW.id_livro; -- NEW.id_livro pega o ID do livro recém-inserido no empréstimo
END //
...
-- -----------------------------------------------------------------------------
-- CADASTRO DOS LIVROS COM CATEGORIA E ESTANTE
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 🔄 EMPRÉSTIMOS E AJUSTE DE STATUS
-- -----------------------------------------------------------------------------
INSERT INTO emprestimos (id_livro, id_leitor, id_funcionario, data_devolucao_prevista, status_emprestimo) VALUES
(1, 1, 1, DATE_ADD(CURRENT_DATE(), INTERVAL 14 DAY), 'Ativo'),
(3, 2, 1, DATE_ADD(CURRENT_DATE(), INTERVAL 14 DAY), 'Ativo'),
(5, 3, 1, DATE_ADD(CURRENT_DATE(), INTERVAL 14 DAY), 'Ativo'),
(8, 1, 1, DATE_SUB(CURRENT_DATE(), INTERVAL 2 DAY), 'Ativo'),
(11, 2, 1, DATE_ADD(CURRENT_DATE(), INTERVAL 14 DAY), 'Ativo');

-- Ajusta o status dos livros 18, 19, 20 e 23 para 'Emprestado'
-- para testar as reservas sem violar o trigger antes_inserir_reserva
UPDATE livro 
SET status_exemplar = 'Emprestado' 
